const peronForm = document.getElementById('person-form')
...
const renderSummary = () => {
    ...
    people.forEach(person => {
        const personExpenses = expenses.filter(exp => exp.paidBy === person).reduce((sum, exp) => sum + exp.amount, 0)
        sum + exp.amount, 0

        const balance = personExpenses - share
